...
export function GastroDetails({ block }: { block: GastroBlock }) {
  ...
  const offerItems = [
    ...
    ...(block.dishTypes?.length ? [{ type: "tagList" as const, label: "Typy pokrmů", items: block.dishTypes }] : []),
    ...(block.dietaryOptions?.length ? [{ type: "tagList" as const, label: "Dietní možnosti", items: block.dietaryOptions }] : []),
    ...(block.foodServiceStyles?.length ? [{ type: "tagList" as const, label: "Styl servisu", items: block.foodServiceStyles }] : []),
    { type: "boolean" as const, label: "Alkoholová licence", value: block.hasAlcoholLicense },
  ...
  return (
    <>
      ...
      <DashboardSection title="Nabídka" icon={Utensils} iconBg="bg-orange-50" iconColor="text-orange-500">
