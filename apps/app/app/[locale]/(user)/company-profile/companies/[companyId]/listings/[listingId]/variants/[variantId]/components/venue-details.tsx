...
export function VenueDetails({ block }: { block: VenueBlock }) {
  const items = [
    ...
    ...(block.includedSpaces?.length ? [{ type: "tagList" as const, label: "Zahrnuté prostory", items: block.includedSpaces }] : []),
    ...
    { type: "boolean" as const, label: "Rezervace celého prostoru", value: block.canBeBookedAsWhole },
    { type: "boolean" as const, label: "Ubytování", value: block.accommodation?.included },
    ...(block.accommodation?.included && block.accommodation.capacity
      ? [{ type: "text" as const, label: "Kapacita ubytování", value: `${block.accommodation.capacity} lůžek` }]
      : []),
    { type: "boolean" as const, label: "Parkování", value: block.parking?.included },
    ...(block.parking?.included && block.parking.spots
      ? [{ type: "text" as const, label: "Kapacita parkování", value: `${block.parking.spots} míst` }]
      : []),
  ...
  return (
    <DashboardSection title="Prostory a vybavení" icon={Package} iconBg="bg-variant-surface" iconColor="text-variant">
