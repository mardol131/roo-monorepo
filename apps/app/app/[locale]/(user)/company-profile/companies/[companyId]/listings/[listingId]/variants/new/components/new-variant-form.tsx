...
const SECTION_BASIC = { id: "section-basic", title: "Základní informace", icon: Building2 };
...
const SECTION_EVENT_TYPES = { id: "section-event-types", title: "Typy akcí", icon: Tag };
const SECTION_AVAILABILITY = { id: "section-availability", title: "Dostupnost", icon: Calendar };
const SECTION_CAPACITY = { id: "section-capacity", title: "Kapacita", icon: Users };
const SECTION_INCLUDES = { id: "section-includes", title: "Zahrnuto / Nezahrnuto", icon: ListChecks };

export const VARIANT_FORM_GROUPS: readonly TocGroup[] = [
  {
    label: "Základní",
    sections: [SECTION_BASIC, SECTION_PRICE, SECTION_IMAGES, SECTION_EVENT_TYPES],
  },
  {
    label: "Konfigurace",
...
type ResolverResult = { values: Partial<VariantFormInputs>; errors: Record<string, unknown> };
...
function makeResolver(): ResolverFn {
  ...
  return async (values, ctx, opts) => {
    ...
    if (v.availability === "selectedHours") {
      const hoursErrors = (result.errors.selectedHours as Record<string, unknown>) ?? {};
      if (!v.selectedHours?.from) {
        result.errors = {
          ...result.errors,
          selectedHours: {
            ...hoursErrors,
            from: { type: "required", message: "Čas od je povinný" },
          },
        };
      }
      if (!v.selectedHours?.to) {
        result.errors = {
          ...result.errors,
          selectedHours: {
            ...((result.errors.selectedHours as Record<string, unknown>) ?? {}),
            to: { type: "required", message: "Čas do je povinný" },
          },
        };
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <Input
                  ...
                  error={errors.price?.seasonalPrices?.[index]?.description?.message}
                />
              </div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <CheckboxGroup
              ...
              items={MOCK_EVENT_TYPES.map((et) => ({ id: et.id, name: et.name }))}
              value={field.value}
              onChange={field.onChange}
      ...
      <FormSection
        ...
        surfaceColor={COLOR_SCHEME.surface}
        color={COLOR_SCHEME.text}
        error={!!(errors.type || errors.availability || errors.selectedHours?.from || errors.selectedHours?.to)}
      >
        {/* Typ sezónnosti */}
        <Controller
          control={control}
        ...
        {availabilityValue === "selectedHours" && (
          <div className="grid grid-cols-2 gap-4">
            <Input
              label="Čas od"
              inputProps={{ ...register("selectedHours.from"), type: "time" }}
              error={errors.selectedHours?.from?.message}
              isRequired
            />
            <Input
              label="Čas do"
              inputProps={{ ...register("selectedHours.to"), type: "time" }}
              error={errors.selectedHours?.to?.message}
              isRequired
            />
          </div>
        )}
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            label={listingType === "venue" ? "Kapacita (osob)" : "Maximální kapacita (osob)"}
            inputProps={{
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button text="Vytvořit variantu" version={BUTTON_COLOR} htmlType="submit" />
      </div>
    </form>
  );
}
