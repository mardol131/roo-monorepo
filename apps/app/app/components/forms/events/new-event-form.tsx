...
import LocationInput from "../../ui/atoms/inputs/location-input";
...
const EVENT_FORM_GROUPS: readonly TocGroup[] = [
  {
    label: "Základní",
    sections: [S.basic, S.budget, S.contact, S.dates],
  },
  {
    label: "Lokalita",
    sections: [S.location],
...
type FormInputs = {
  ...
  locationDistrict?: { id: string; name: string } | null;
...
function validate(values: Partial<FormInputs>): FieldErrors<FormInputs> {
  ...
  if (!values.locationDistrict)
    e.locationDistrict = { type: "required", message: "Vyberte město" };
...
const resolver: Resolver<FormInputs> = (values) => {
  ...
  if (Object.keys(errors).length === 0) {
    return { values: values as FormInputs, errors: {} };
  }
...
export { resolver as eventFormResolver };
  ...
  return {
    ...
    locationDistrict:
      typeof loc.district === "string"
        ? { id: loc.district, name: "" }
        : { id: loc.district.id, name: loc.district.name },
...
export default function NewEventForm({
  ...
  const buildLocation = (data: FormInputs): Event["location"] => ({
    district: data.locationDistrict!.id,
  ...
  const fillContactInfoFromUserHandler = useCallback(() => {
    ...
    setValue("contactPerson.name", name, { shouldValidate: true });
    setValue("contactPerson.email", email, { shouldValidate: true });
    if (countryCode && number) {
      setValue("contactPerson.phone.countryCode", countryCode, {
        shouldValidate: true,
      });
      setValue("contactPerson.phone.number", number, { shouldValidate: true });
    }
  }, [user, setValue]);
            ...
            <Controller
              ...
              render={({ field }) => (
                <GuestsInput
                  ...
                  error={
                    errors.guests?.adults?.message || errors.guests?.message
                  }
                />
              )}
            />
