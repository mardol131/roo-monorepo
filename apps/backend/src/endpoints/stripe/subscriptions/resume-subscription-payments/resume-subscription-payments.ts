...
export const resumeSubscriptionPaymentsEndpoint: Endpoint = {
  path: '/stripe/subscriptions/resume',
  method: 'post',
  handler: async (req: PayloadRequest) => {
    ...
    if (!doc) {
      return Response.json({ error: 'No cancelling subscription found for this listing' }, { status: 404 })
    }
    ...
    try {
      ...
      req.payload.logger.error({ err }, 'Failed to resume Stripe subscription')
      return Response.json({ error: 'Failed to resume Stripe subscription' }, { status: 502 })
    }
